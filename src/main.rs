...
impl Buffer {
    ...
    fn next(&mut self) {
        let chars: [&str; 5] = ["TONY.", "PRESCOTT.", "GRANDPA.", "MURIEL.", "ASHLEY."];
...
fn load_file() -> Vec<String> {
    let mut s = death_by_golf::get_text();
...
fn App(cx: Scope) -> Element {
    ...
    cx.render(rsx! {
        p{ "Currently - {char.read().charature.clone()} - {buffer.read().page_name.clone()}"}
        div {
            height:"55vh",
            overflow:"auto",
